			...
			match self.socket.read(&mut buf) {
				Ok(size) => {
					if size == 0 {
						...
						split_message(&mut self.buffer, self.id, queue_o, &buf[..size]);
					}
				}
				Err(err) => {
					if let WouldBlock = err.kind() {
...
#[inline]
fn get_length(buf: &[u8], start: usize) -> usize {
	(
...
pub fn split_message(buffer: &mut Vec<u8>, id: i32, queue_o: &Queue<Message>, buf: &[u8]) {
	macro_rules! queue_o {
		($data:ident) => (
			queue_o.push(doc!{
				"event": "sys:recv",
